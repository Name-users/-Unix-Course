...
struct Client
{
    int inner_state_update;
    long int inner_state;
    long int seq_number;
    size_t length;
...
struct FDArray * get_FDArray()
{
    ...
    array->length = 0;
    array->poll_array = NULL;
    array->client_array = NULL;
    return array;
}
...
void del_client(struct FDArray *array, int index)
{
    ...
    {
        fprintf(stderr, "ERROR: функция del_client: некорректный индекс\n");
        exit(1);
    }
...
void cl_sleep(double time)
{
    ...
    while (1)
    {
        ...
        {
            fprintf(stderr, "ERROR: ошибка выполнения функции cl_sleep\n");
            exit(1);
        }
...
int receive_number(int fd, long int *number)
{
    ...
    if (rec == -1)
    {
        fprintf(stderr, "ERROR: Ошибка получения данных\n");
        return 0;
    }
    else if (rec == BUFFER_SIZE + 1)
    {
        ...
        {
            fprintf(stderr, "ERROR: Получены неккоректные данные '%s'\n", buffer);
            return 0;
        }
    }
    else
    {
        fprintf(stderr, "ERROR: Получено только %ld байт данных\n", rec);
        return 0;
    }
    return 1;
}
...
int send_char(int fd, char ch)
{
    ...
    if (sent == -1 || sent == 0)
    {
        fprintf(stderr, "ERROR: Ошибка отправки символа\n");
        return 0;
    }
    return 1;
}
...
void signal_handler(int s)
{
    ...
    fprintf(stderr, "INFO: Сервер завершен, внутреннее состояние: %ld\n", inner_state);
    exit(0);
}
...
void server()
{
    ...
    {
        fprintf(stderr, "ERROR: Ошибка настройки обработки сигналов SIGINT/SIGTERM\n");
        exit(1);
    }
    ...
    while (1)
    {
        ...
        if ((events = poll(fd_array->poll_array, fd_array->length, 100)) > 0)
        {
            if (fd_array->poll_array[0].revents & POLLIN)
            {
                ...
                {
                    fprintf(stderr, "ERROR: Ошибка принятия клиента\n");
                    continue;
                }
                struct Client cd;
                ...
                fprintf(stderr, "INFO: Используется файловый дескриптор %d для нового подключения\n", cfd);
                fprintf(stderr, "INFO: Указатель на границу кучи: %p\n", sbrk(0));
            ...
            for (int i = fd_array->length - 1; i > 0; i--)
            {
                ...
                {
                    fprintf(stderr,
                            "ERROR: Не удалось установить соединение с клиентом под номером %ld\n",
                            fd_array->client_array[i].seq_number);
                    del_client(fd_array, i);
                    continue;
                }
                ...
                if (revents & POLLIN)
                {
                    if (!fd_array->client_array[i].inner_state_update)
                    {
                        ...
                        {
                            fprintf(stderr, "ERROR: Ошибка получения символа от клиента под номером %ld\n",
                                    fd_array->client_array[i].seq_number);
                        }
                        else
                        {
                            ...
                            if (ch == '\0')
                            {
                                ...
                                {
                                    fprintf(stderr, "ERROR: От клиента %ld получено некорректное число '%s'\n",
                                            fd_array->client_array[i].seq_number,
                                ...
                                {
                                    inner_state += number;
                                    fprintf(stderr, "INFO: От клиента %ld получено число %ld\n",
                                            fd_array->client_array[i].seq_number,
                                            number);
                                    fprintf(stderr, "INFO: Обновлено внутреннее состояние сервера: %ld\n", inner_state);
                ...
                if (revents & POLLOUT)
                {
                    if (fd_array->client_array[i].inner_state_update)
                    {
                        ...
                        {
                            fprintf(stderr, "ERROR: Ошибка отправления внутреннего состояния клиенту %ld\n",
                                    fd_array->client_array[i].seq_number);
                        }
                        else
                        {
                            fd_array->client_array[i].inner_state_update = 0;
                            fprintf(stderr, "INFO: Отправлено внутреннее состояние %ld клиенту %ld\n",
...
void client(double delay)
{
    ...
    {
        fprintf(stderr, "ERROR: Ошибка установления соединения с сервером\n");
        exit(1);
    }
    fprintf(stderr, "INFO: Соединение с сервером установлено\n");
    ...
    while (1)
    {
        ...
        {
            fprintf(stderr, "ERROR: Ошибка poll");
            break;
        }
        if (events > 0)
        {
            ...
            if (revents & POLLHUP)
            {
                fprintf(stderr, "INFO: Сервер закрыл соединение\n");
                break;
            }
            if (revents & POLLERR)
            {
                fprintf(stderr, "ERROR: Ошибка соединения с сервером\n");
                break;
            }
            if (revents & POLLNVAL)
            {
            ...
            if ((revents & POLLIN) && (recv_server_is))
            {
                ...
                {
                    fprintf(stderr, "ERROR: Ошибка получения внутреннего состояния сервера\n");
                    close(cfd);
                    exit(1);
                }
                else
                {
                    fprintf(stderr, "INFO: Внутреннее состояние сервера: %ld\n", number);
                }
                c_pollfd.events = POLLOUT;
                recv_server_is = 0;
                continue;
            ...
            if ((revents & POLLOUT) && (!recv_server_is))
            {
                ...
                {
                    fprintf(stderr, "INFO: На сервер отправлен символ '%c'\n", ch != '\0' ? (char)ch : ' ');
                }
    ...
    close(cfd);
    fprintf(stderr, "INFO: Сумма всех задержек: %lf секунд\n", delays_sum);
    fprintf(stderr, "INFO: Соединение с сервером завершено\n");
}
...
int main(int argc, char *argv[])
{
    ...
    if (argc == 4)
    {
        ...
        else if (strcmp(argv[1], "-c") == 0)
        {
            ...
            {
                fprintf(stderr, "ERROR: Ошибка открытия конфигурационного файла\n");
                exit(1);
            }
            close(fd);
        ...
        {
            fprintf(stderr, "ERROR: Некорректные аргументы командной строки\n");
            help();
            exit(1);
        }
    }
